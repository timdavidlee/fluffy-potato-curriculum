# L12: Teacher-led demos — What an agent generates: state, logs, traces & extracts

> Sibling doc: [objectives.md](objectives.md) (what the lesson aims for), parent design [CURRICULUM_PRD.md](../../CURRICULUM_PRD.md) (lesson-plan row L12).
> Preceding lesson demos: [L10 demos_or_activities.md](../L10/demos_or_activities.md) (the ReAct **graph** these demos instrument). Following lesson: [L13 Evaluation: first pass](../L13/objectives.md) (consumes the trace this lesson produces).
>
> **Audience for this file:** the teacher running L12. Every demo below is *teacher-driven, no student participation*. Student-driven exercises live in the L12 labs (separate file, produced by stage 2).
...
- **If the demo misbehaves** — graceful fallback for when the model surprises you (because it will; the agent is non-deterministic and that non-determinism is itself part of objective 4).

The demos are ordered to follow the learning objectives and the lesson's **concept-first, then tooled** spine, bracketed by the objective-6 taxonomy: **Demo 0** draws the opening map — *what a run generates, across two planes* (objective 6, the frame); Demo 1 motivates *why* a structured trace beats a streamed `print()` (the L10 stream beat, now this lesson's opener) and produces the first tiny trace; Demo 2 *reads* a full trace and reconstructs the run — including the **state** the trace carries (objective 1); Demo 3 *locates a failure* from the trace alone (objective 2); Demo 4 *instruments* the graph live — capturing its `stream` — to emit the full `TraceEvent` trace (objective 3); Demo 5 *compares two traces* of the same task (objective 4); Demo 6 *exports the same run to self-hosted Langfuse* and re-does objectives 2 and 4 in a real dashboard (objective 5); **Demo 7** closes the frame — *extracts go to a store, not the trace* (objective 6, conceptual). They build on each other — Demos 2, 3, and 5 read traces of the same graph Demo 4 instruments, so students see the artifact before they see how it is produced (concept first), then meet the real tool last (then tooled); Demos 0 and 7 are the conceptual bookends that place all this tracing work on the wider map. Run them in order on first delivery.

> **A note on what L12 reuses from L10, and what is new.** L12 does **not** re-teach or re-derive the L10 graph — the ReAct cyclic `StateGraph` (agent node, `route`, `ToolNode`, back-edge) is assumed (see the L10 "Bridge to L12" and this lesson's objectives "L10 overlap" decision). What L12 adds is *observation*: a structured trace captured from the graph's own `stream`, read after the fact. Per the objectives' "inline-build vs. reference" decision, the **canonical reference copy** of the graph and tools lives in the shared `common/` layer — `common/agent_graph.py` (`build_agent(...)` + the `trace_graph(...)`/`run(...)` producer), `common/tools.py`, and `common/tracing.py` (`TraceEvent`); `common/agent_loop.py` is kept alongside as a plain-`while`-loop reference on the same contract, but the graph producer is L12's subject. The demos below reference the graph and tools by name as a stable import, not as live-coded scratch (the one live-code beat is Demo 4's stream-to-span wrapper).

## Naming and tooling reconciliation (read before building any demo)

The shared `common/` modules these demos import **now exist** (foundation landed). The names below are the shipped ones; use them verbatim so the demo scripts, `common/`, and L13 agree.

- **Producer name.** The traced subject is `common/agent_graph.py`: `build_agent(model, tools, *, handle_tool_errors=True)` compiles the L10 ReAct graph, and `trace_graph(graph, user_msg, *, max_steps=8)` / `run(model, tools, user_msg, max_steps=8)` run it and return a `RunResult`. The demos below use **`agent_graph.run(...)`** (or `trace_graph(...)` when a pre-built graph is on hand). `common/agent_loop.py`'s `run(...)` is the same-signature plain-`while`-loop reference kept alongside — mention it as "the by-hand version," don't make it the subject.
- **`RunResult` carries a `trace` field.** `RunResult` is `final_text: str`, `iterations: int`, `termination: str` (`"natural"` | `"max_steps"`), and **`trace: list[TraceEvent]`** — shared by both producers. `iterations` counts `agent`-node visits; `termination` is `"natural"` on a clean stop (`route` → `END`) and `"max_steps"` when the graph's `recursion_limit` trips a `GraphRecursionError`. Demo 2's narration of "where each `RunResult` field came from in the trace" reads directly off this.
- **`flaky_fetch` exists in `common/tools.py`.** The shared tools are `calculator(expression)` (raises `ValueError` on non-arithmetic input), `lookup(city)` (raises `KeyError` for a city not in `POPULATIONS`), and `flaky_fetch(url)` with four URL behaviors (`https://ok` success, `https://error` structured error-as-data, `https://crash` raises `RuntimeError`, `https://garbage` malformed output). `ToolNode(handle_tool_errors=True)` converts any raised exception into a `ToolMessage(status="error")`. Between them these give objective 2 all four failure signatures with no new tools.
- **Anchor model.** Decided in the objectives: Claude **Sonnet 4.6**, inheriting the L01–L10 precedent so a trace of the `common/` graph matches the behavior students saw in L10. Used live in Demos 4–6; Demos 1–3 read pre-captured traces and need no live call.

## Pre-flight (once, at the top of the lesson)

The teacher should have, before the first demo starts:

- A working notebook/REPL with the project `uv` env, able to `from fluffy_potato_curriculum.common import agent_graph, tools, tracing` (the shared reference modules). Keys load through `common/config.py` (the pydantic-settings seam), never hard-coded — consistent with the project's live-by-default notebook stance.
- **A set of pre-captured traces, saved to disk as `.jsonl`** (one `TraceEvent` span per line, via the decided `.to_jsonl()` helper), captured *before class* on Sonnet 4.6 so the reading demos (2, 3, 5) are deterministic and don't depend on a live model behaving on the day:
  ...
  - <!-- *NEED INPUT*: confirm where these captured traces live in the repo (a demos/ subdir under the L12 lesson folder? a fixtures file?) and that they are committed so the teacher isn't re-capturing live before every class. The compare demo (5) especially benefits from a saved A/B pair with a known, explainable difference. -->
- **The exact chaining task** used to produce `trace_good.jsonl`. It must force a *multi-step* run (at least two tool calls plus a natural-termination text turn) so there is something to narrate. Use L10's chaining task — *"Population of the city that is 17\*17 - 1? Use the tools."* — driven by a scripted `FakeModel` (`calculator("17*17-1") → 288`, then `lookup("Tokyo") → 37000000`, then a text answer): two `tool` spans, three `agent` spans, natural termination. The `FakeModel` makes the chain deterministic (the model doesn't have to *derive* Tokyo from 288 — the script decides it), which is exactly why the reading demos are reproducible.
...
2. Walk the **observability plane** as an increasing-permanence axis: **state** (the live message history the graph feeds the model — mutating, in-memory, gone at exit), **logs** (the streamed `{node: update}` play-by-play — human-readable), **traces** (the durable, structured, run-scoped record). Say the through-line: *"a trace is mostly state, captured durably over time — that's why reading a trace is reading what the model saw."* All three answer **"what did the run do?"** and are read by *you and the eval harness*.
...
## Demo 1 — From a streamed `print()` to a structured event (Objective 3, opener; reinforces the L10 stream beat)

**Goal:** motivate the whole lesson in two minutes. Land the main point *"structured beats printed the moment you have two runs"* by turning one of L10's streamed `{node: update}` print lines into a structured event appended to a list — the simplest possible trace. This is the L10 stream-inspection beat, promoted to L12's opener per the objectives' "L10 overlap" decision.

**Pre-flight:**

- L10's stream loop visible in scrollback or on a slide: `for chunk in graph.stream(task, stream_mode="updates"): ...` printing one line per node visit (agent → tools → agent → …). Recall it by name as the *"minimum-viable trace"* and *"tracing on-ramp"* L10 already called it.
- An empty `events: list[dict] = []` ready to append to.

**Live script:**

1. Recall L10's stream out loud: *"L10 ended with this — one printed line per node visit, straight off `graph.stream(stream_mode='updates')`. We called it a minimum-viable trace. Watch what's wrong with it the moment we have more than one run."*
2. Run the graph's stream with the print. Read the printed lines. Then ask the rhetorical question: *"Now compare this run to yesterday's run. How? Scroll up and eyeball two walls of text?"*
3. Replace **one** `print(...)` inside the stream loop with a structured dict appended to a list — e.g. `events.append({"run_type": "tool", "name": msg.name, "inputs": args})`. Run again. Show `events` at the end: a list of records, not a wall of text. Note that you added this *around* the stream — the graph itself is untouched.
4. Land the contrast explicitly: a printed line is human-readable and ephemeral; a record can be **filtered, counted, diffed, and fed to evaluation** (forward-point to L13). That jump — printed chunk → structured record — is the single most important instrumentation move in the lesson.

**What to highlight:**

- A trace is the **durable memory of an ephemeral run.** The graph runs and exits; without a record, all you keep is the final answer and whatever scrolled past. Say this first and say it plainly — it's the reason the lesson exists.
- *"Tracing is just logging"* is the confusion to pre-empt right here. A pile of streamed lines is not a trace until it has **order, structure, and a run identifier**. This demo shows structure; Demo 4 adds the run identifier.
- This dict is a hand-rolled stand-in; Demo 4 replaces it with the real `TraceEvent` Pydantic model from `common/tracing.py` (and the full `trace_graph` mapping). Don't polish the dict — it's a teaser.

**If the demo misbehaves:**

- Nothing model-dependent here; one streamed run is enough. If a live call is slow, run it against one of the pre-captured traces instead and just show the difference between the printed line and the structured record.
...
2. For each span, name **which kind** it is from `run_type` (`llm` / `tool` / `chain`) and show where one model→tool round ends and the next begins — an `llm` span is one `agent`-node visit, a `tool` span one `ToolNode` dispatch.
...
5. Close by distinguishing the three things students now have in front of them: a **log line** (one event), a **trace** (the ordered record of the whole run), and the **`RunResult`** (the run's summary). Point at where each `RunResult` field — `final_text`, `iterations`, `termination` — *came from* in the trace.
...
- These four signatures are exactly L13's first eval cases (objective 2 of L13: "design eval cases that target failure modes already seen in traces from L12"). Tell students to keep the failures they find — they become regression cases next lesson.
...
## Demo 4 — Instrument the graph to emit a real trace (Objective 3)

**Goal:** turn the toy dict from Demo 1 into the real thing — capture the graph's `stream` into `TraceEvent` spans so `agent_graph.run(...)` returns a populated `RunResult.trace`. Land that **instrumentation is a wrapper, not a rewrite**: the L10 graph is untouched — you can't reach inside a compiled graph, so you observe its stream from the outside.

**Pre-flight:**

- The `common/agent_graph.py` producer open — `build_agent(...)` (the compiled L10 graph) and the `trace_graph(...)` stream-to-span mapping, ideally with the mapping folded behind a flag or shown incrementally. This is the one live-code beat of the lesson.
...
1. State the rule before typing: *"I am not changing how the graph decides anything — I couldn't if I wanted to; it's compiled. I'm reading its stream and turning each node visit into a span."* Point at the stream: `for chunk in graph.stream(task, stream_mode="updates")`.
2. Live-add the mapping. For each `{node: update}` chunk, append a `TraceEvent`: an `agent` update → one `llm` span (carrying the reply's `tool_calls`, `usage` token counts, `start_time`/`end_time`); a `tools` update → one `tool` span per `ToolMessage` (carrying `name`, the `inputs` recovered from the preceding `agent` reply's tool call, and `error` on failure); plus the `chain` framing span. Give every span the **same `trace_id`** so they belong to one run — call out that this `trace_id` is the field that makes Demo 5's comparison and Demo 6's Langfuse view possible.
...
- The agent is **non-deterministic** (L10's "variance budget"). The same task can take a different path through the graph each run. A diff is how you tell a real regression from normal variation — but you must know what variation is *normal* before you call a difference a regression. Pre-empt *"the two traces are different, so I broke something"* head-on.
- This is the **seed of evaluation.** Comparing runs by eye to flag a regression is exactly the discipline L13 formalizes into a repeatable eval set — *same comparison, scaled from one eyeball diff to a fixed set of cases run many times*. Forward-point to L13 by name.
...
- **Live-coding budget:** Demo 4's instrumentation wrapper is the *only* place to live-code. Demo 1 is a one-line edit; Demos 2, 3, 5 read fixed files; Demo 6 runs an export, not new logic. Do not re-derive the L10 graph anywhere — it's imported (`common/agent_graph.build_agent`).
...
- **The audience watches, doesn't participate.** Resist *"where's the bug in this trace?"* as a class question — that's the L12 lab pattern, not a demo pattern. Hands-on trace-reading is for the labs (stage 2).

## Open authoring questions

- **Resolved:** producer/tool naming is settled — the traced subject is `common/agent_graph.py`'s `run(...)`/`trace_graph(...)` over `build_agent(...)`, tools are `common/tools.py`'s `calculator`/`lookup`/`flaky_fetch`, `RunResult` already carries `trace`. See "Naming and tooling reconciliation" above.
...
- **Resolved:** the multi-step chaining task for Demo 2 is L10's *"Population of the city that is 17\*17 - 1?"* run on a scripted `FakeModel` (`calculator("17*17-1")` → `lookup("Tokyo")` → text) — deterministic and runnable against `common/tools.py` today.
- <!-- *NEED INPUT*: the A/B framing for Demo 5's curated trace pair — prompt before/after vs. same-task-twice vs. model-vs-model — bearing in mind model-vs-model (Sonnet vs Haiku) is L13's headline contrast and may be best left to L13. -->
