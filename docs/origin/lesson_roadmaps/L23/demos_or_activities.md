...
- **Demos host on the repo's real `.claude/skills/` — DECIDED (2026-07-04): yes.** The worked examples are the actual skills — author-lesson-roadmap → generate-materials-from-roadmap (handoff), the `.claude/rules/*.md` (rubric archetype), sync-lesson-numbering (shared-node cousin) — running on the L04/L11 LangGraph agent with `list_skills` + `load_skill` LangChain tools, live only for the model-selection demos (4 and 5). No new runtime dependency beyond LangGraph.
