...
- **The repo's real skills open in an editor**, ready to read on screen:
  ...
  - `.claude/rules/python-style.md`, `.claude/rules/pytest.md` — real **review/rubric (principle-centric)** content the project applies as judgment. <!-- DECIDED (2026-07-04): fair to present the `.claude/rules/*.md` files as the "rubric archetype" worked example — they're real, on-disk, principle-centric guidance the repo's agents actually apply, even though they aren't frontmatter-wrapped skills. Call them "rubric-style guidance"; note that packaging one as a `SKILL.md` with a discriminating description is exactly Demo 2's authoring exercise. -->
- **The LangGraph skill-loading agent**: the **[L03](../L03/objectives.md)/[L11](../L11/objectives.md) LangGraph agent** (Sonnet 4.6) extended with two **LangChain tools** that implement JIT loading — **`list_skills()`** returns every registered skill's `{name, description}` (discovery), and **`load_skill(name)`** reads that skill's full `SKILL.md` body into context (load). The model **calls `list_skills` to see what's available, then `load_skill` to pull the one it needs.** This is L22's JIT mechanism realized *as agent tools* on the agent students built in L11 — not a hand-rolled loader. L23 runs the agent live only where the model actually *selects* a skill (Demo 4's JIT-across-the-graph, Demo 5's description collisions); Demos 1–3 are mostly *reading and graphing real skills*. <!-- DECIDED (2026-07-01): runtime = the L03/L11 LangGraph agent with `list_skills` + `load_skill` LangChain tools (Sonnet 4.6), not L22's hand-rolled loader. L22 hand-rolls to demystify; L23 does it 'properly' as tools, which also makes each dependency-graph edge concrete (an edge = one skill's body calling `load_skill` on another). No new runtime dependency beyond LangGraph, in use since L03. -->
...
- **Two prepared "bad" skills** for Demo 5's anti-patterns:
  ...
  <!-- DECIDED (2026-07-04): Demo 5 material = a colliding-description pair (run live) + a shared-skill-that's-really-a-tool stub + a hand-*drawn* over-chained pipeline (5-hop vs. 2-step). Over-chaining stays drawn, not runnable — a design smell best shown on the graph. The broader effort of giving every mini-cut lesson a gotcha/anti-pattern demo (using this Demo 5 as the template) is tracked separately: docs/todos/2026-07-01-mini-cut-gotcha-antipattern-demos.md. -->
...
3. Open the **script-centric** example built for this lesson — `src/fluffy_potato_curriculum/lessons/L23/example_skills/get_order/SKILL.md` — a thin markdown wrapper whose real work is a bundled script (`get_order_api.py`): "to fetch an order, run the script and parse the JSON it prints." Name it: **API/integration recipe** — center of gravity is the *script*. Run it live once (`uv run python -m fluffy_potato_curriculum.lessons.L23.example_skills.get_order.get_order_api ORD-1001`) so students see the messy JSON contract the wrapper hides. <!-- DECIDED (2026-07-04, todo #9): the repo now ships a real script-centric example under the lesson (deliberately not in `.claude/skills/` — it's teaching material, kept out of the live registry): `get_order/SKILL.md` + a mock `get_order_api.py` with a heterogeneous JSON contract (discriminated-union payment, per-item options, optional tracking). -->
...
- A shared-skill candidate — **now built for this lesson:** `src/fluffy_potato_curriculum/lessons/L23/example_skills/check_lesson_links/SKILL.md`, a small skill both operating skills would call. `author-lesson-roadmap` (validating cross-refs in a roadmap) and `generate-materials-from-roadmap` (validating cross-refs in generated materials) both `load_skill` it rather than re-implementing the check. It delegates the mechanical part to a bundled `extract_links.py` and keeps the *judgment* (broken ref vs. illustrative `L<NN>` forward-pointer vs. not-yet-generated sibling) in the skill — which is why it's a shared skill, not a tool. `sync-lesson-numbering` is the nearest real cousin. <!-- DECIDED (2026-07-04, todo #9): the shared lower-level skill is real, built as `example_skills/check_lesson_links/` (out of the live `.claude/skills/` registry). It pairs a mechanical helper (`extract_links.py`) with `SKILL.md` judgment, modelling the "shared skill, not a tool" line from the good side. On its first live run it surfaced three off-by-one broken links in this roadmap, since fixed. -->
- The L03/L11 LangGraph agent with `list_skills`/`load_skill` and all skills registered. Token readout visible.
...
- **Per-demo time:** Demo 1 (classify real skills) 12–15 min. Demo 2 (author per archetype) 15–20, the biggest live-authoring block. Demo 3 (sequential handoff) 12–15. Demo 4 (shared skill + graph + JIT) is the centerpiece, 18–25 (drawing + token readout). Demo 5 (anti-patterns) 12–18. Optional bridge 3–5. Total ~75–100 minutes plus discussion. <!-- DECIDED (2026-07-04): duration pinned in objectives.md at ~90–120 min — single session by default, with an OPTIONAL split at the Demo 2/3 boundary: Demos 1–2 = lecture A ("archetypes & authoring"), Demos 3–5 = lecture B ("composition, the graph & anti-patterns"). This per-demo budget (~75–100 min of demo time) sits inside that window. -->
...
- **Build on L03/L11, not L22's loader:** the runtime is the LangGraph agent from L03/L11 plus two small tools (`list_skills`, `load_skill`); L22's hand-rolled loader was the demystifying version and is *not* reused here. L23 spends its time on *composition*, not on re-deriving the loading mechanism.
...
- **Demos host on the repo's real `.claude/skills/` — DECIDED (2026-07-04): yes.** The worked examples are the actual skills — author-lesson-roadmap → generate-materials-from-roadmap (handoff), the `.claude/rules/*.md` (rubric archetype), sync-lesson-numbering (shared-node cousin) — running on the L03/L11 LangGraph agent with `list_skills` + `load_skill` LangChain tools, live only for the model-selection demos (4 and 5). No new runtime dependency beyond LangGraph.
- **Real example skills — DONE (2026-07-04, todo #9).** The two archetype gaps are filled with real, runnable, offline artifacts under `src/fluffy_potato_curriculum/lessons/L23/example_skills/` (lesson-owned, deliberately *not* in the live `.claude/skills/` registry): a script-centric `get_order` skill over a mock orders API with a heterogeneous JSON contract, and a shared `check_lesson_links` skill (mechanical `extract_links.py` + `SKILL.md` judgment). Both have mirrored tests and pass the full gate. See [example_skills/README.md](../../../../src/fluffy_potato_curriculum/lessons/L23/example_skills/README.md).
...
- **Lab continues the L22 skill — DECIDED (2026-07-04): yes.** The lab (stage 2) reuses each student's L22 skill as one node of the L23 system — growing it into a handoff or adding a shared sub-skill ("grow your one skill into a small system"). Mirrors the objectives.md decision.
...
- **L22 overlap guardrail — DECIDED (2026-07-04): holds.** Demos *reuse* L22's single-skill authoring, description-as-trigger, and JIT-loading as callbacks, not re-teach them (L22 = author one skill well; L23 = classify archetypes + compose many). Demo 5's "shared skill that's really a tool" is the one intentional re-invocation of L22's skill-vs-tool line.
- **L24 overlap guardrail — DECIDED (2026-07-04): holds.** The optional bridge demo only *forward-points* to agent composition (relabel the Demo 4 graph); it never builds a multi-agent system, so L24 isn't pre-empted.
