...
4. Without re-running, walk the room through the three failure modes from [objectives.md](objectives.md):
   ...
   - **Quality degradation** near the long-context tail — even when a call fits, models can struggle to attend to material in the middle of a very long context. Foreshadow that this motivates the techniques in L14 (context management) and L15 (RAG).
...
- "Bigger window" does not mean "use it all." More tokens = more cost, more latency, and (often) lower quality near the tail. Foreshadow L09 (model power) and L14.
...
- Bridge: the cost-awareness mindset installed here is what makes L14 (context management), L09 (model power), and prompt-caching design moves *land* later. Without L01, those lessons feel like premature optimization.
