...
2. **Reason about context windows.** Concretely:
   ...
   - Foreshadow why this matters later: every multi-turn agent (L10+) is fighting the context window, and the techniques in L19 (context management) and L21 (RAG) exist to push back against that pressure.
...
3. **Explain temperature and sampling.** Concretely:
   ...
   - Pick a temperature for a task and defend the choice: classification or extraction → low; creative writing or brainstorming → higher; agent tool-routing (preview of L07) → low.
...
4. **Estimate cost.** Concretely:
   ...
   - Estimate the cost of a multi-turn conversation, accounting for the fact that prior turns are re-sent on every call (no server-side memory by default — a fact that surprises most students and motivates everything from prompt caching to L16/L17).
   ...
   - Order-of-magnitude estimate the cost of an agent run (preview of L10/L12) — e.g. "a 10-step agent with 2k-token context per step on Sonnet costs roughly $X" — without needing exact numbers.
...
- **The context window is a hard ceiling on everything the model considers at once — input *and* output combined.** Students often think of "the prompt" as the only thing that consumes window. In reality the system message, prior turns, tool definitions (relevant from L07 onward), and the model's own response all share the same budget. This single fact drives the design of half the techniques in this course.
...
- *"Bigger context window = always better."* No: more tokens cost more, more tokens slow inference down, and models can degrade in quality near the long-context tail. The course revisits this in L12 and L16.
...
- <!-- *NEED INPUT*: is prompt caching introduced here (as a foreshadow when discussing the "every call re-sends history" cost surprise), or strictly deferred to L19 (context management)? -->
- <!-- *NEED INPUT*: should "rate limits" (RPM/TPM, retry/backoff) sit in L01 alongside cost, or be deferred until students are running multi-step agents and actually hit them in L10? -->
