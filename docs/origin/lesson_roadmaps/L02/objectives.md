...
L02's three subgoals — roles, structured output, few-shot examples — are the minimum prompting toolkit a student needs before L06 can teach reasoning techniques on top of them. L06 ([objectives](../L06/objectives.md), [demos](../L06/demos_or_activities.md)) explicitly assumes the student arrives able to send multi-turn role-segmented chats, request structured output, and read its own few-shot examples. Everything from L07 onward (tool calling, agents) presumes the same toolkit.

This lesson is foundational and *narrow on purpose*. It does not teach chain-of-thought, scratchpad reasoning, or self-critique — those are L06. It does not teach tool calling or function-calling protocols — that is L07. The temptation to broaden scope here should be resisted; L02's job is to hand L06 a competent multi-turn chat user, nothing more, nothing less.
...
- <!-- *NEED INPUT*: should L02 introduce prompt caching as a concrete technique (it most naturally lands when discussing the "system message is durable, user message is per-call" split in Subgoal 1), or strictly defer to L19 (context management)? Mirrored from [L01 objectives](../L01/objectives.md) where it was raised as an L01-vs-L16 question. -->
