...
> Note on lesson numbering: in the full 20-lesson plan, **L10 (Choosing model power for the task)** precedes L11. It is dropped from the mini cut, so L11 follows L09 directly. If L10's roadmap is later authored, revisit this lesson's model-selection asides — L11 deliberately keeps "which model to use" out of scope (it anchors on **Sonnet 4.6** for clarity), and that choice should hand cleanly to L10 in the full course. **Decided:** L11 anchors on a single model (Sonnet 4.6) and defers all model-power trade-offs to L10; in the mini cut, model selection is simply not covered.
...
- **Decided (anchor model):** Claude **Sonnet 4.6** — the same model L07's demos used, so the LangGraph rebuild is directly comparable to the hand-rolled original. L11 uses a single model; model-power trade-offs are L10's job (full course, out of the mini cut).
