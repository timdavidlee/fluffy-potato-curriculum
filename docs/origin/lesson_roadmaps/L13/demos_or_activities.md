...
>
> **Anchor model for the demos: Claude Sonnet 4.6** (inherits the L01–L10 precedent so the traced graph behaves as students saw it). **Contrast model: Claude Haiku 4.5**, used in Demo 3 as a second experiment over the same dataset to make a lower-powered model's pass rate visibly drop.
>
...
- **The shared agent graph importable, not rebuilt.** `from fluffy_potato_curriculum.common.agent_graph import run` (the producer that runs L10's ReAct graph and traces it) and the shared tools from `common/tools.py` (`calculator`, `lookup`, `flaky_fetch` with its four URL behaviors). L13 *evaluates* a graph students already built and traced. `agent_graph.run(...)` returns a `RunResult` carrying `final_text`, `iterations`, `termination`, and — from L12 — `trace: list[TraceEvent]`. (`common/agent_loop.py`'s same-signature `run` is the plain-`while`-loop reference kept alongside; it produces the identical `RunResult` contract if a demo ever wants to contrast the two producers, but the graph is the subject.)
...
> Why reuse L10/L12's graph, tools, tasks, *and* Langfuse instance: L13 is about *eval design*, not new agent code, new tools, or a new platform. Evaluating a graph students already traced, in the Langfuse they already use, keeps every minute on the eval concepts — the case, the scorer, the pass rate, the cost, the judge — which *are* the lesson.
...
1. **Ground it in real data first.** Open the Langfuse trace for one experiment run and read the *actual* per-call `usage` token counts. Note that **each case is a *full* `agent_graph.run(...)`** — multiple model calls (one per `agent`-node visit), not one — and multiply through for the dataset (N cases × K samples × calls/run).
...
- **Variance budget:** the agent is non-deterministic — that's the *subject* of Demo 3, not just a nuisance. Budget a re-run wherever a specific tool-call path matters, and **capture the saved failure traces ahead of class** so Demo 2 never depends on reproducing a failure live.
