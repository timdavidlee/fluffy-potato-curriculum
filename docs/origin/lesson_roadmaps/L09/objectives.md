...
> Preceding lesson: [L08 Tracing — reading what your agent did](../L08/objectives.md). Following lesson: L10 Choosing model power for the task (roadmap not yet written; in the mini cut the next taught lesson is [L11 Explicit graphs & workflows in LangGraph](../L11/objectives.md), then [L12 Shallow agents in LangGraph](../L12/objectives.md)).
...
This is also the lesson the PRD singles out as a *practice to carry forward*: "establish the practice that every later lesson will carry forward." After L09, every agent students build — the LangGraph shallow agent in L12, and beyond — should come with at least a minimal eval set. L09's job is to make that habit cheap enough to keep.

L09's eval harness stays deliberately **small and hand-rolled**: students write a handful of eval cases and a tiny runner in plain Python against the *same hand-rolled loop they have been tracing*. (This is "concept-first, then tooled," same as L08 — L08 hand-builds a trace before meeting Langfuse; L09 hand-builds an eval harness before any platform eval.) Hosted eval platforms — including the **datasets/experiments feature of the same self-hosted Langfuse** students already use — and LLM-as-judge at scale are named as forward pointers but not taught here; L22 (Evaluation revisited, outside the mini cut) is where evaluation is generalized to multi-step, RAG, and multi-agent systems. **Decided:** L09's eval harness stays a tiny hand-rolled Python harness students read end-to-end; Langfuse-backed and large-scale LLM-judge eval are deferred to L22.
...
5. **Establish evaluation as a practice every later lesson carries forward.** Concretely:
   - State the discipline as a rule students will reuse: *when you build or change an agent, you add or run an eval set.* Frame the L09 harness as the seed that L12's LangGraph agent (and every agent after) plugs into.
   - Recognize what L09 is *not*: it is a *first pass* — a small, hand-rolled harness — not a production eval system. L22 (Evaluation revisited) scales the same discipline to multi-step graphs, retrieval quality (precision@k / recall@k), LLM-as-judge, and multi-agent systems. Knowing where the first pass stops is part of the objective. **Decision:** the eval harness — `EvalCase`, the `Scorer` protocol, and the runner — is promoted to `common/evals.py` (alongside L08's `common/tracing.py` and the shared `common/tools.py`), so L12 and later lessons import it rather than redefine it. (The exact *case shape / scorer interface* is still being pinned — see open questions.)

## What an eval set is (vocabulary the lecture must establish)

Define these explicitly and reuse them verbatim through the labs and into L12/L22:
...
- **This is a first pass, on purpose.** A tiny readable harness over the hand-rolled loop, not a platform. It establishes the *habit*; L22 scales the *machinery*. Naming that boundary keeps the lesson honest and the scope small.
...
- *"An LLM-as-judge is a free way to check anything."* It's a scorer with its own cost and its own errors — it can be wrong, biased, or gameable. L09 only name-drops it; L22 unpacks what it can and can't reliably score.

## Bridge to L12 (and forward to L22)

In the mini cut, the next taught lesson is **L11 (Explicit graphs & workflows / DAGs)**, then **L12 (Shallow agents in LangGraph)** — and L12 is where the eval handoff lands. The handoff is the *practice*, not a specific artifact: when students rebuild the agent as a LangGraph graph in L12, they should bring an eval set with them and run it against the new implementation. (L11's deterministic workflow is itself trivially evaluable — a natural, cheap place to reinforce the habit even earlier.) That is the cleanest possible demonstration of L09's ratchet — *same cases, different implementation, did anything regress?* — and it makes "evaluate every agent you build" concrete the very next lesson. **Decided:** L12's lab reuses the L09 eval harness (`common/evals.py`) against the LangGraph agent — already settled by the shared-`common/` decision; this is the strongest reinforcement of the "carry it forward" objective.

Looking further ahead, **L22 (Evaluation revisited)** — outside the mini cut — extends this first pass to systems L09 deliberately doesn't touch: per-node vs. end-to-end metrics for multi-step graphs, retrieval quality (precision@k / recall@k) for RAG, LLM-as-judge done properly, and multi-agent evaluation. L09 should explicitly signpost L22 as "where this scales," so students read L09's smallness as a starting point, not a ceiling.

> Tooling forward pointer: the same **self-hosted Langfuse** instance students met in L08 (and reuse in L12) also has a datasets/experiments feature that stores eval runs and scores — i.e. the platform version of L09's hand-rolled `evaluate()`. L09 stays the hand-rolled harness (so students see the mechanism), with Langfuse-backed evaluation named as a forward pointer toward L22, consistent with the L08 "concept first, then tooled" stance. **Decided:** Langfuse-backed eval is a name-drop only here; the full treatment is L22.

> Note on lesson numbering: in the full plan, L10 (Choosing model power) sits between L09 and L11. In the mini cut, L10 is dropped, so L09 hands directly to L11 (workflows), then L12 (agents). If the L10 roadmap is later authored, revisit this bridge — L09's cost-and-model-class reasoning (objective 4) is a natural lead-in to L10's model-power trade-offs. **Decided:** in the mini cut the bridge points at L11→L12; L10 is mentioned only as a parenthetical, so the doc serves both the mini and full-course delivery without rework.

## Open authoring questions

- **Decided (lecture duration):** **~75–100 minutes**, one lecture, including a code-along that builds the runner and turns two L08 trace failures into regression cases. If it runs long, split into "build an eval set / score answer vs. trajectory" and "compare runs / reason about cost".
...
- **Decided (reuse tools):** L09's cases reuse L07/L08's tools and tasks from `common/tools.py` (`calculator`, `lookup`, `flaky_fetch` with its four URL behaviors). Evaluating a loop students already traced keeps the focus on eval design, not new tools, and `flaky_fetch`'s failure modes give objective 2 ready-made regression cases.
- **Decided (LLM-as-judge depth):** show **one minimal illustrative judge** — a small judge prompt scoring a `final_text` — for the cost/judgment-spectrum discussion, clearly flagged as "the L22 version is more rigorous." Enough to make the spectrum concrete without expanding scope.
- **Decided (L08 overlap):** L09 **reinforces and formalizes** L08's by-eye two-trace comparison into a repeatable eval-set comparison (regression flagging); it does **not** re-teach trace reading — that skill is assumed from L08, not re-derived.
