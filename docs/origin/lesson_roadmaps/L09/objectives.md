...
L09 stays deliberately **small and framework-free**, matching L07/L08: students write a handful of eval cases and a tiny runner in plain Python against the *same hand-rolled loop they have been tracing*. Hosted eval platforms and LLM-as-judge at scale are named as forward pointers but not taught here — L20 (Evaluation revisited, outside the mini cut) is where evaluation is generalized to multi-step, RAG, and multi-agent systems. <!-- *NEED INPUT*: confirm L09 stays framework-free and defers all hosted-eval / large-scale LLM-judge tooling to L20. Recommendation: yes — L09 is "first pass," built on the hand-rolled loop; keep it a tiny Python harness students can read end-to-end. -->
...
### Decided eval schema (approximate, LangSmith-shaped)

**Decision:** like the L08 trace, the eval types are a deliberately *approximate* match to LangSmith's evaluation model — close enough that a student who later uses a real eval platform recognizes the structure, but a tiny hand-rolled Python harness, not the SDK. Authored in `common/evals.py` (shared `common/` layer), consuming the `RunResult.trace` from `common/tracing.py`.
...
- <!-- *NEED INPUT*: which Claude model anchors the demos/labs — mirrors the L07/L08 open question. A Sonnet-4.6-vs-Haiku-4.5 contrast could double as the objective-3 A-vs-B regression comparison, since the cheaper model's pass rate on the same eval set is a vivid teaching moment. -->
- **Decided (case/scorer representation):** approximate LangSmith-shaped schema — `EvalCase` (Example: `inputs` + `reference_outputs`), `Scorer` (Evaluator: `(run, example) -> EvalResult{key, score, comment}`), `evaluate(..., samples=K)` returning per-case pass rates. Authored in `common/evals.py`, consuming `RunResult.trace`. See *Decided eval schema* above.
