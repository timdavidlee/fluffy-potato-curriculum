...
> Preceding lesson: [L13 Evaluation: first pass](../L13/objectives.md). Following lesson: L15 LangGraph design patterns (roadmap not yet written). **Track:** L14 is **full-track only** — the mini cut drops it (in the mini plan L13 hands directly to L03), so this lesson can assume the full arc around it.
...
- Build and run an agent as a graph — a forward workflow (L03/L05) and the ReAct cyclic graph (L10), or its `create_agent` form ([L11](../L11/objectives.md)) — and understand that a **node** is the unit of orchestration (L03). L14 binds models *at the node*, so a student shaky on the node abstraction should revisit L03 & L05 first.
...
- **Decided (track):** L14 is **full-track only** — the mini cut drops it (in mini, L13 hands directly to L03). Keep the lesson self-contained so the full track reads continuously; do not make any later *mini* lesson depend on L14.
...
- <!-- *NEED INPUT*: which graph the hands-on binds models onto — an L03/L05 workflow vs the L10 agent (see objective 3) — and the provider keys / offline fallback for a live mixed-provider run (see objective 3's second marker). -->
