...
- <!-- *NEED INPUT*: how deep on the schema spec — JSON Schema fully, or just the subset the JSON schema `bind_tools` infers from a typed function (via `convert_to_openai_tool`) actually validates? Affects how much time the lecture spends on schema syntax vs. design principles. -->
