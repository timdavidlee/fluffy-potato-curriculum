...
3. Register it alongside two tools the agent runs but does not author: the reused `calculator` (total the line items, compute the reconciliation variance against the matched record) and the **provided** `check_expense_policy` (imported pre-built from `receipt_tools.py` — an **LLM read** of the free-form policy prose that judges the expense and cites the rule). Three tools give the agent a *real tool-selection decision* to make — which makes the Segment 4 trace worth reading. Frame the policy tool in one line (the policy is free-form prose, so it's a single LLM read that cites the rule — the slice's one *LLM-in-the-loop* tool, warranted by the same L08 test; offline it runs on a scripted judge, live Sonnet in class); do **not** author it live — the writing budget stays one tool.
