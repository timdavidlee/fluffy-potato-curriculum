...
>
> **⚠️ Split note (reorder 2026-07-02) — mirrors [objectives.md](objectives.md)'s split note.** This demo file was the former **L12** "Explicit graphs & workflows," which demoed prompt-chaining **and** routing in one lesson. Under the reordered plan L04 is split: **L04 keeps sequential prompt-chaining** (Demo 1 + the Demo 4 workflow-vs-agent close), and **routing / conditional edges move to the new L05** ([../L05/demos_or_activities.md](../L05/demos_or_activities.md), now authored, which owns the routing demos). Concretely: **Demo 2 (routing + mixed models) and Demo 3 (user-input branching) below describe routing that now belongs to L05** — they stay here only until the **stage-2 materials pass** regenerates L04 as *sequential-only* and builds L05's routing demos. (Per-node **model mixing** itself is L04-native and already shown in Demo 1's `parse`=Haiku / `draft`=Sonnet chain, independent of Demo 2's routing wrapper — only the routing moves.) The **built L04 materials (`L0401`–`L0404`) are already sequential-chaining-only** and forward-reference routing as L05's, so this file is the last pre-split L04 artifact. The **coda below already reflects the post-split scope** (L04's two sequential-DAG gotchas; the two routing gotchas moved to [L05's coda](../L05/demos_or_activities.md#common-pitfalls-coda--naming-l05s-three-gotchas)). Until the regen, read Demos 2–3 as L05 source material parked here — L05 is the source of truth for routing.
