...
- The agent loop or any cyclic control flow — that's L10 (hand-rolled) and L14 (LangGraph); nothing in L03 loops.
- Real trace tooling — Langfuse arrives at L11. If students want to "watch the node run," this lesson uses LangGraph's own built-in invoke/stream output, not a tracing platform.
- Any mixed-model decision framework — that's L13. If a cheap-vs-capable model contrast comes up at all in this lesson, it is a single forward-pointing sentence, not a taught idea.
...
- *"A `StateGraph` with one node is already an agent."* No. Nothing here loops, and nothing here lets the model choose what happens next — there isn't a "next" yet. Agency is introduced at L14 with a back-edge that lets the *model* decide to keep going; L03 has no edges to speak of.
...
- *"I need to see this node's trace / spans to know it worked."* Not yet — full trace tooling is L11. For one node, `invoke()`'s return value and LangGraph's own stream output are enough to answer "did it run, and what came back."
...
- <!-- *NEED INPUT*: should this lesson's node use Sonnet 4.6 only (matching the course anchor and keeping the model constant so "one node" is the only variable), or is a brief, non-taught Haiku 4.5 aside worth including as a preview of L04's per-node model mixing? Current draft assumes Sonnet-only to avoid pulling L13/L04 content forward. -->
