...
- **Reasoning is not free.** Every CoT or scratchpad token is paid for, latency-incurring, and competing with other content for the context window. L03 is the first lesson where the student must consciously weigh reasoning quality against cost — a theme that returns in L10 (model power) and L17 (context management).
...
- <!-- *NEED INPUT*: should self-critique introduce the idea of using a *different* model as the critic, or defer that to L10 (model power)? -->
