...
- **Reasoning is not free.** Every CoT or scratchpad token is paid for, latency-incurring, and competing with other content for the context window. L06 is the first lesson where the student must consciously weigh reasoning quality against cost — a theme that returns in L14 (models & providers) and L19 (context management).
...
L07 introduces tool calling, which adds an outer loop: the model decides whether to answer directly or call a tool. That decision is itself a reasoning step. Everything from L06 — eliciting reasoning, separating reasoning from output, recognizing when reasoning is wasted — applies inside the tool-calling loop, especially when reasoning about *which* tool to call and whether the tool's response is sufficient to answer.
...
- <!-- *NEED INPUT*: should self-critique introduce the idea of using a *different* model as the critic, or defer that to L14 (models & providers)? -->
