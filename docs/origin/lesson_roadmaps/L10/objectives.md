...
1. **Build a ReAct agent as a cyclic `StateGraph`, node by node.** Concretely:
   ...
   - Run the compiled graph on a small concrete task (e.g. "use the calculator and the lookup tool to answer X") and read the returned `messages` list: agent → tools → agent → tools → agent (final text). Name where the cycle turned.
