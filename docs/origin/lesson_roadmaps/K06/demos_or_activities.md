# K06: Self-paced walkthrough — Docker & the multi-container stack

> Sibling docs: [objectives.md](objectives.md), parent design [CURRICULUM_PRD.md](../../CURRICULUM_PRD.md).
>
> **Audience:** the *student*, alone at a keyboard. Same "Do → You should see → why it matters → If
> it breaks" shape as [K01](../K01/demos_or_activities.md). This is the **last prework unit** — the
> finish line (Step 6) is the go/no-go gate for starting `L01`.
...
that actually persist data — see the open question in [objectives.md](objectives.md).)
...
ability to read typed/pydantic/async code), you are **cleared to begin `L01`.**
...
`<need input: if a repo-owned local-Langfuse compose overlay is wanted (a pinned compose.langfuse.yaml
in this repo rather than a pointer to the upstream one), decide the pinned Langfuse version and
author it here — weigh the maintenance/drift cost the vendor note in classroom-llm-management.md
flags.>`
