...
| [`todos/`](todos/) | **Dated, point-in-time tracking notes** (`YYYY-MM-DD-<slug>.md`) — short lists of open items for one effort. | You want to **capture open items / follow-ups** discovered during a work session. Cheap and disposable. |
...
- **Naming:** dated files in `todos/` and `handoffs/` use `YYYY-MM-DD-<kebab-slug>.md`.
