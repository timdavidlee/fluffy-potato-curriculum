...
- [ ] **L07 — Tool calling.** Gotchas: forgetting the **schema is in every request** (tokens twice over);
...
- [ ] **L08 — Designing good tools.** Anti-patterns: **over-tooling** (a tool for what the model can do
      alone); vague tool **name/description**; **unhandled tool errors**; one tool doing too much (no clean
      schema).
- [ ] **L10 — Hand-rolled agent loop.** Gotchas: **no termination condition** (infinite loop) / no
      max-iterations guard; **not handling tool failures**; **unbounded context growth** across turns.
- [ ] **L11 — Tracing.** Anti-patterns: tracing **too little** (can't locate the failure) vs **too much**
      (noise, prompt bodies, PII); **not instrumenting tool calls**; reading spans out of order. *(bake in
      when the demos doc is authored)*
- [ ] **L12 — Evaluation.** Anti-patterns: eval set that only tests the **happy path**; **sample too small**;
      **over-trusting LLM-as-judge**; not targeting **failure modes seen in L11 traces**; regressions that
      slip through. *(bake in when the demos doc is authored)*
- [ ] **L04 — Explicit graphs & workflows (DAGs).** Gotchas: using a **workflow where an agent is needed**
...
- [ ] **L14 — Shallow LangGraph agent.** Gotchas: **state mismanagement** (messages vs counter / reducers /
      typing); **no termination**; conflating **prebuilt vs hand-assembled** `StateGraph`. *(bake in when the
      demos doc is authored)*
- [ ] **L22 — Skills.** Anti-patterns: description **too vague** (never triggers) or **too broad** (always
