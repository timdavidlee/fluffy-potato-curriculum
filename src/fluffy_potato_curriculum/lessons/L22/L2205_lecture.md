...
- diagram: side by side — your `Skill` catalog + the `load_skill` tool on your L11
  `create_agent` on the left; a `SKILL.md` file under `.claude/skills/` loaded by Claude Code on
  the right — with `=` between them.
