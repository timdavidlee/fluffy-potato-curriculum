...
Notes for whoever runs the L05 labs. One section per problem, keyed by lab id and problem number.
Times are rough and assume a semi-technical student with basic Python who completed L01–L04.

> Cross-cutting note: the four teacher demos (L0503 tool-or-no-tool / L0505 the description / L0507
> schemas + validation errors / L0509 errors & side effects) make **live** Claude calls and need
> `ANTHROPIC_API_KEY` set (copy `.env.example` to `.env`). Dry-run them before class — model
> behavior **varies**, and several demos (L0505's sparse-vs-rich, L0509's retry) are *distributional*,
> so run the named variants twice. Clear any API-touching cell outputs before committing.
>
> **All four L05 labs are OFFLINE / pure Python** — no API key required. L05 is about *design
> judgment*, and you practice that by designing schemas, rewriting errors, and classifying tasks, not
> by spending tokens. The labs map to the four subgoals: **L0504** → tool-or-no-tool; **L0506** →
> name + schema design; **L0508** → error shapes; **L0510** → idempotency + side effects.
>
> **Why the demos use the raw Anthropic SDK and not `potato_llm`:** the course's `potato_llm` seam is
> text-only — its `Message` cannot carry `tool_use`/`tool_result` blocks, which L05's demos need in
> order to watch the model *choose* and *call* tools. So the L05 demo notebooks call the raw SDK
> directly (the key still loads through `common.config`), exactly as L04 does. The labs don't touch
> the SDK at all. (Open course question, same as L04: extend `potato_llm` to model tool blocks, or
> keep L04+ demos on the raw SDK.)
>
> L05 assumes the L04 mechanics work. If a student is shaky on the tool-call round-trip, redirect to
> the L04 lab before this lesson — L05 does **not** re-teach the protocol.
...
- **Key point:** this tiny validator is the *shape* line of defense — it does not (and can't) check
  whether a well-formed value is *true* (L0508/L0509's job).
...
- **Unblockers:** "Just append `(to, body)` and return `len(sent)`. Call it twice on the same `log`;
  expect length 2." This mirrors the L0509 demo's OUTBOX growing to two entries.
- **Time:** ~5 min.
...
- **Unblockers:** expected: something like *\"If the user does not exist, this tool will create one
  ...
  the model can reason about it before calling. Forward-link to L12 (approval gates).
- **Time:** ~4 min.
