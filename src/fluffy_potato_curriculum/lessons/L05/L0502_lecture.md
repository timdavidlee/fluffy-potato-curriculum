...
> ([L0503](L0503_lecture.ipynb) tool-or-no-tool, [L0505](L0505_lecture.ipynb) the description,
> [L0507](L0507_lecture.ipynb) schemas + validation errors, [L0509](L0509_lecture.ipynb) errors &
> side effects); hands-on practice is in the L05 labs (L0504 / L0506 / L0508 / L0510).
> **Anchor model throughout: Claude Sonnet 4.6** (one Haiku 4.5 contrast in L0505).

## section 1. The lesson in one claim

### slide 1.1 From making a tool work to making a tool good

- L04 settled the *mechanics*: how a single tool wires to a model call, the round-trip on the wire,
  the protocol the model and runtime use to negotiate a call. By the end of L04 you can make a tool
  *work*.
- L05 is about a different thing entirely: making a tool *good*. The protocol is fixed — the open
...
- table: the four design decisions L05 builds, the one sentence each lands, and the demo + lab that
  drills it.

| Decision | The mental model | Demo / lab |
| --- | --- | --- |
| Tool, or no tool? | adding a tool is architecture, not convenience | [L0503](L0503_lecture.ipynb) / L0504 |
| The description is the tool | same name + schema, different description → different behavior | [L0505](L0505_lecture.ipynb) / L0506 |
| Schemas are a contract | a tight schema converts ambiguity into recoverable errors | [L0507](L0507_lecture.ipynb) / L0508 |
| Errors are the interface | an error is a prompt for the model's next turn | [L0509](L0509_lecture.ipynb) / L0510 |
...
- table: the four "reach for a tool" signals from [objectives.md](../../../../docs/origin/lesson_roadmaps/L05/objectives.md),
...
- The discipline: **run the task model-alone first**, see where it actually fails, and add a tool only
  ...
  [L0504 lab](L0504_lab_empty.ipynb) has you classify a table of tasks and defend each call.
...
- table: the three error classes from [objectives.md](../../../../docs/origin/lesson_roadmaps/L05/objectives.md)
...
- The [L0508 lab](L0508_lab_empty.ipynb) has you take raw stack traces and rewrite each into an
...
- [Demo 4](L0509_lecture.ipynb) shows a `lookup_user_by_email` that *also silently creates* a user:
  ...
  ask first.
- Forward link: **L12 (human-in-the-loop / approval gates)** revisits this exact tension for
  high-stakes side effects. L05 plants the seed; L12 builds the structure.
...
- table: the recurring student confusions from [objectives.md](../../../../docs/origin/lesson_roadmaps/L05/objectives.md)
...
## section 9. Bridge to L06

### slide 9.1 The design survives the packaging change

- L06 introduces **MCP (Model Context Protocol)**, a portable packaging format that lets the *same*
  tool be exposed to many clients without rewiring.
- Everything L05 teaches — naming, descriptions, schemas, error shapes, idempotency — applies
...
- A poorly designed tool exposed via MCP is still a poorly designed tool — just available to more
  clients. L06 leans on the L05 vocabulary throughout.
...
- **Idempotency and side effects** → still a runtime concern in an MCP world, and still must be named
  in the description.
- One sentence to L06: *you can now design a tool a model uses well; next you'll package that same
