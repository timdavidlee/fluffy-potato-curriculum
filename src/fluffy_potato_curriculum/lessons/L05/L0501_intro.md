...
> **Lesson:** L05 — Designing good tools.
...
> [L0505](L0505_lecture.ipynb) to show the design gap widens on a smaller model.

## Where this lesson sits

L04 covered the *mechanics* of tool calling: how a single tool is wired to a model call, what the
tool-call round-trip looks like on the wire, and the protocol the model and runtime use to negotiate
a call. By the end of L04 a student can make a tool *work*.

L05 takes the next step: making a tool *good*. The protocol is settled — the open question now is
...
Every design choice in L05 follows from that one sentence.

## The four judgment calls L05 builds

L04 gave you the wiring. L05 gives you four design decisions you make *every time* you add a tool.
Each maps to one learning objective and one demo + lab pair.

- **Tool, or no tool?** ([Demo 1](L0503_lecture.ipynb) / [L0504 lab](L0504_lab_empty.ipynb)) — adding
...
- **The description is the tool.** ([Demo 2](L0505_lecture.ipynb) / [L0506 lab](L0506_lab_empty.ipynb))
...
- **Schemas are a contract, not a suggestion.** ([Demo 3](L0507_lecture.ipynb) /
  [L0508 lab](L0508_lab_empty.ipynb)) — the model is a *fuzzy* producer of structured output
...
- **Errors are part of the interface.** ([Demo 4](L0509_lecture.ipynb) /
  [L0510 lab](L0510_lab_empty.ipynb)) — the model treats a tool error as *new context*, not as an
...
## Three mental models to carry out of L05
...
- **L03 (reasoning is tokens).** Choosing *whether* to call a tool is itself a reasoning step. A good
  description and a clean error are the tokens the model conditions that decision on.
- **L04 (the protocol).** L05 assumes the wiring works. We do **not** re-teach the round-trip — if you
  are shaky on the mechanics, redirect to the L04 lab before continuing. L05 is about *design
...
plain text, not tool-use or tool-result blocks. L05's demos register tools and observe the model
*choose* and *call* them, so the demo notebooks drop down to the **raw Anthropic SDK**
(`client.messages.create(..., tools=[...])`), exactly as [L04](../L04/L0401_intro.md) introduced. The
...
## What L05 deliberately does *not* teach

- **Not the MCP wire format.** L06 packages these exact design choices — names, descriptions, schemas,
  error shapes — as a portable contract across clients. Everything L05 teaches applies **unchanged** to
...
- **Not the agent loop.** Composing one tool's output into the next call is what L07 (the hand-rolled
  agent loop) makes first-class. L05 designs the individual tool well so L07's loop has good parts to
  work with.

The one sentence to leave L05 with:
...
(L0503 / L0505 / L0507 / L0509) and the hands-on labs (L0504 / L0506 / L0508 / L0510).
