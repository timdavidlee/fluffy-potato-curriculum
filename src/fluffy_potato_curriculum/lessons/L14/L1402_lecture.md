...
- A **shallow agent** is a *single tool-calling loop*: **one** model, **one** set of tools, **one**
  decision point that either calls a tool or finishes. It is the L10 loop, now a graph.
...
- *Deep* agents add planning, persistent memory, sub-todos, and reflection — a heavier, later
  choice (L18). Named here only as "what we are **not** building yet."
...
- The pieces, named with the vocabulary you reuse into L15:
...
### slide 2.3 Map every piece back to an L10 line
...
- The **conditional edge is the L10 branch**, lifted out of Python control flow into the graph.
...
- The payoff shows up when the control flow stops being one loop — branches, multiple tools nodes,
  sub-graphs — which is precisely the motivation for **L15 (patterns)**.
...
- The **state** is a typed object threaded through every node. For a shallow agent it is
  essentially the running **message history** — the same list L10 mutated — plus one `step` counter
...
- The `agent` node calls the model (bound with the tool schemas) on the current messages and
  ...
  the matching `tool_result`s — *the same message-history invariant from L10*, now localized to one
  node.
...
def agent_node(state: AgentState) -> dict[str, object]:
    """The L10 'call the model' step, as a node."""
...
- For the `tools` node we use LangGraph's **prebuilt `ToolNode`** (section 5) — it runs each
  `tool_use` and appends a `tool_result`, the L10 dispatch step packaged for you.

### slide 3.3 Wire it, compile it

- The same `StateGraph` recipe as L04, plus the **back-edge**:
...
def route(state: AgentState) -> str:
    ...
    # the L10 'is there a tool_use?' branch, now a routing function
...
### slide 3.4 Run it — behavioral equivalence with L10

- Invoke the compiled graph on the **L10 chaining task** ("compute, then look up a population"):
...
### slide 4.2 `add_messages` is what preserves the L10 invariant

- L10 made you *manually* keep every `tool_use` paired, in order, with its `tool_result`. Get it
  wrong and the model loses the thread — the single most common L10 bug.
...
### slide 4.3 Break the reducer on purpose (the L10 bug, reborn)
...
- Conflating the two is how a graph gets tangled. Keep the running agent's state to **messages plus
  one counter**; richer state is L15's (patterns) and L19's (context management) territory.

## section 5. What the framework gives you for free

### slide 5.1 Name each freebie against its L10 twin

- table: the scaffolding L10 made you write by hand, now supplied. That convenience *is* the value
  proposition — name each freebie against its hand-rolled twin so the trade is concrete, not magic.

| L10 made you write… | LangGraph supplies… |
| --- | --- |
...
- We build the graph with **explicit nodes and edges first** (sections 3–4), so the L10→graph
...
- A runaway graph still halts: LangGraph's **recursion limit** is the framework's `max_steps`.
  Hitting it is still a **signal worth investigating** (the L10 lesson), not noise.
- `ToolNode` does **not** swallow arbitrary tool failures by default — you opt in with
  ...
  That is the L10 `dispatch()` "convert any exception into a `tool_result(is_error=True)`" behavior,
...
- For a *single* shallow loop, the graph is roughly break-even versus L10's plain Python — honestly,
  a bit more ceremony for the same behavior.
- It earns its keep when control flow **branches** (L15 patterns), or when you want the framework's
...
- Name it out loud: **over-reaching for a framework on a 50-line problem** — the L10-objective-4
...
- The engineering skill is choosing the **simplest shape that solves the task**: a single function
  when one will do, the L10 loop when you want to *see* every moving part, a graph when the control
...
- **Model an agent as a graph** — name each of `agent` node, `tools` node, edge, conditional edge,
  state, reducer, entry point / `END`, and the L10 piece each replaces (objective 1).
- **Build a single-loop LangGraph agent** — assemble the `StateGraph`, wire the back-edge, compile,
  run it to behavioral equivalence with L10, and recognize the prebuilt one-liner (objective 2).
