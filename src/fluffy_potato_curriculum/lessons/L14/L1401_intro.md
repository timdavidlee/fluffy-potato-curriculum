...
with a structured trace ([L11](../L11/objectives.md)) and to *judge* it with a minimal eval set
([L12](../L12/objectives.md)).

L14 cashes that in. You rebuild the **same agent you already understand** as a **LangGraph graph**.
...
*you* wired every edge and the model never decided what ran next. L14 takes those exact same
...
**`ChatAnthropic`** directly, **not** the hand-rolled `potato_llm` seam from L01–L12 — "frameworks
...
skills from L11–L12: the **same** L12 eval set runs against this rebuild (it passes → the rebuild is
correct), and the graph's run lands in the **same** L11 Langfuse dashboard. Evaluation and tracing
