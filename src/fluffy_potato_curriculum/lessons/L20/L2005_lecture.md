...
> **Lesson:** L20 (the mini cut's **final lesson** — a capstone).
> **Roadmap:** [demos_or_activities.md](../../../../docs/origin/lesson_roadmaps/L20/demos_or_activities.md).
...
- Same payoff as the rest of the course's spine: L08's hand-rolled trace was Langfuse; L12's
  hand-rolled loop was LangGraph; **L20's hand-rolled loader is Agent Skills.** You met the
...
- That single question ties together everything you built this course: prompts (L02), tools
  (L04/L05), the agent loop (L07), and its LangGraph form (L11/L12).

### slide 5.2 The forward pointer (full course only — don't teach it here)

- In the full course, L20 bridges to **L22 (multi-agent / subagents)**: a *subagent* is
  capability-as-a-whole-agent — the same "where should capability live" lens, scaled from a skill up
  to an entire agent.
- The broader "keep a long-running agent's context lean" problem is **L17 (context management)**;
  the fuller capability-packaging taxonomy (with MCP) is **L06**. Name them; the mini cut ends here.
