...
use the instructor-issued key if your cohort provides one):
...
missing or in the wrong place, you get a clear sentence right away instead of a cryptic `KeyError`
deep in a call.
...
`potato_llm` seam (this is the same kind of smoke call K06's final go/no-go uses). Keep it tiny:
low `max_tokens`, one call.

```sh
uv run python -c "
from fluffy_potato_curriculum.potato_llm import AnthropicClient, Message
reply = AnthropicClient().chat([Message.user('Say hi in one word.')], max_tokens=16)
print(reply.text)
"
```
