...
Notes for whoever runs the L01 labs. One section per problem, keyed by lab id and problem number.
Times are rough and assume a semi-technical student with basic Python.
...
- **Unblockers:** the key is `enc.decode([id])` for each `id` individually (note the single-element
  list), joined with `|`.
- **Time:** ~5 min.
...
- **Common gotchas:** `o200k_base` missing on a very old `tiktoken` — the repo pins a current
  version, so this should be fine, but if it errors, any second encoding makes the point.
...
- **Common gotchas:** leaving `ratio = 0.0`; expecting JSON to obey the ≈4 rule.
- **Unblockers:** "chars per token = characters ÷ tokens" — they already have both numbers.
- **Time:** ~5 min.
- **Key point:** JSON is far below 4 chars/token because structural characters each cost a token;
  the ≈4 rule is an English-prose rule.

## L0107_lab problem 1 — Headroom
...
## L0107_lab problem 2 — Match the failure mode
...
## L0107_lab problem 3 — Cost of one call
...
## L0107_lab problem 4 — The input/output asymmetry

- **Common gotchas:** expecting the larger *total* token count to be pricier; the point is the
  *direction*, not the total (both calls are ~2,050 tokens).
...
## L0107_lab problem 5 — The conversation-history staircase
...
- **Unblockers:** the shape is a list comprehension calling
  `client.chat([Message.user(PROMPT)], max_tokens=16, temperature=temperature).text` n times.
- **Time:** ~7 min.
...
## L0109_lab problem 4 — Why isn't temperature 0 perfectly deterministic?
