...
> **Lesson:** L11. **Roadmap:** Demo 4 in [demos_or_activities.md](../../../../docs/origin/lesson_roadmaps/L11/demos_or_activities.md).
> This is the closing lecture — mostly **diagram + discussion**, no live build. It reuses one of the
> compiled workflows from the demos ([L1103](L1103_lecture.ipynb) / [L1105](L1105_lecture.ipynb)) and
> *names* the one change that makes an agent. Building the agent is **L12's** job.
> **Anchor:** the workflow-vs-agent contrast, stated verbatim so it carries into L12.
...
- diagram: the L1103 chain `parse → draft → policy_check → END` with a finger-trace arrow showing
...
- table: the L11 primitives — and the note that **L12 reuses every one of them unchanged.**

| Primitive | What it is | Reused in L12? |
| --- | --- | --- |
...
- L12 keeps **all** of the above and adds exactly one thing: a **conditional edge that loops back
...
- diagram: the acyclic chain on the left; on the right the same shape with one new **dashed
  back-edge** curving from the model node to `tools` and back — labeled "the only thing L12 adds."
...
- That model → tools → model loop is exactly the one you **hand-rolled in [L07](../L07/objectives.md)**
  — only there the `while`/`if` was plain Python, and in L12 it's a graph edge.
- diagram: side by side — L07's `while` loop pseudocode and L12's cyclic graph — captioned "same
  loop, now expressed as a graph."
- So L12 is not a new world: it's **L11's primitives + L07's loop**, written as one back-edge.

### slide 2.3 A conditional edge is still not "the model is an agent" by itself

- Be precise: in L11 a conditional edge branched on **state the developer set** (a label, a user
...
- **Determinism is a feature, not a limitation.** A workflow takes the same path on the same input:
  predictable, cheaper, lower-latency, and trivially testable (which is why the L09 eval set drops
  straight onto it).
...
## section 4. Bridge to L12

### slide 4.1 What carries forward

- Everything. `StateGraph`, nodes, edges, typed state, reducers, `compile`/`invoke`, and the
  Langfuse hookup all reappear in L12 **unchanged**.
- L12 also re-teaches the primitives from scratch so the agent lesson stands alone — treat the
