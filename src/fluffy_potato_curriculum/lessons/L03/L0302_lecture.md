...
> **Lesson:** L03. **Roadmap:** [objectives.md](../../../../docs/origin/lesson_roadmaps/L03/objectives.md).
...
> ([L0303](L0303_lecture.ipynb) CoT, [L0305](L0305_lecture.ipynb) scratchpad,
> [L0307](L0307_lecture.ipynb) self-critique, [L0309](L0309_lecture.ipynb) when-it-hurts); hands-on
> practice is in the L03 labs (L0304 / L0306 / L0308 / L0310).
...
- L02 taught the *shape* of a prompt — roles, structured output, few-shot. L03 teaches a technique
...
### slide 1.3 The four techniques L03 hands you
...
- This with/against comparison is exactly what the L0304 lab has you build, and what
...
- **Evals & tracing** — a clean answer field is what L08 (tracing) and L09 (evaluation) score against;
...
| Different **model** | a second model has different failure modes (revisited rigorously in L09) |
...
- L03 is the first lesson where you must consciously weigh reasoning *quality* against *cost*. That
  trade-off returns in L09 (model power) and L14 (context management).
...
- Given a task, an accuracy requirement, and a latency/cost budget, **decide and defend**: reason, or
  answer cold? That decision *is* the L03 skill — the L0310 lab has you make it on a table of tasks.
...
## section 6. Bridge to L04

### slide 6.1 Reasoning becomes a step inside a loop

- L04 introduces **tool calling**, which adds an outer loop: the model decides whether to answer
  directly or call a tool. *That decision is itself a reasoning step.*
- Everything from L03 applies inside that loop — eliciting reasoning about *which* tool to call,
...
- **Reasoning is tokens** → a tool call is also just tokens the model emits in a structured shape
  (L04's central framing — the parallel is deliberate).
- **Parse defensively** → tool-call arguments can be malformed exactly like a broken `<answer>` tag;
  the same discipline applies.
- One sentence to L04: *you can make the model think before it answers; next you'll let that thinking
