...
> **Lesson:** L03 — Directed graphs: from one node to a sequential chain (**Movement 2**, the
> chaining half). **Roadmap:** [objectives.md](../../../../docs/origin/lesson_roadmaps/L03/objectives.md).
> This page is your written reference for the chaining movement — thorough on purpose, so if you
> missed the live session you can rebuild it from here. It follows the build-one-node demo
> ([L0303_lecture.ipynb](L0303_lecture.ipynb)); the live chaining demo is
> [L0305_lecture.ipynb](L0305_lecture.ipynb), and you get hands-on practice in the L03 lab
> ([L0306](L0306_lab_empty.ipynb)). [L05](../L05/objectives.md) picks these same primitives up and
> adds a conditional edge.
> **Anchor model: Claude Sonnet 4.6** (heavy nodes), **Claude Haiku 4.5** (light nodes) — the
> chaining movement deliberately mixes models per node.

## section 1. The lesson in one claim

### slide 1.1 One node becomes several, wired in order

- Earlier in this lesson you built a `StateGraph` at its smallest: one typed node. Now you'll wire
  **several** such nodes into a fixed sequence — a **workflow**, not an agent.
- From L01–L02 your control flow was plain Python: a single call, then (Movement 1) one wrapped
  node. Here you'll turn a *sequence* of steps into a **graph**: explicit nodes wired by edges *you*
  lay out. The model lives *inside* the nodes; it never decides what runs next.
...
- diagram: the single `node` box on the left growing into the fixed chain on the right — one
  **ink-faint** box `node` (where you were, Movement 1), an arrow, then three **cyan** wired boxes
  `parse → draft → policy_check` into an ink-faint `END`, captioned "one node → several wired in
  order." This chain is the lesson's motif — every diagram from here re-draws it with one thing
  changed.
...
- Every edge you write in this lesson is **fixed** — `A → B`, always. There's nothing to branch on yet;
  that arrives in L05.
...
## section 2. The StateGraph primitives (vocabulary, continued from Movement 1)
...
- You built exactly this recipe earlier this lesson with **one** node and one edge straight to
  `END`. Here you're using the same recipe with **several** nodes and edges *between* them — new
  wiring, no new primitive.
...
- A node does **one unit of work** and hands back an update — nothing more. That's unchanged from
  the single node you just built; what's new here is that *several* nodes now share one state object.
...
- **State** is a typed object (a `TypedDict`) threaded through every node. In Movement 1 your state
  had two fields and one node touching them; here your state is threaded through **several** nodes in
  turn.
- A **reducer** is the rule that merges a node's returned update into state, *per field*. The
  ...
  first genuinely new primitive in Movement 2 — you never needed one at a single node (nothing to
  merge).
...
- **Edge** — a fixed transition, `A → B`, taken every time: `add_edge("parse", "draft")`. Every
  edge in this lesson is this kind; L05 adds a second kind (the conditional edge).
...
### slide 5.2 Watch it run — `graph.astream`, the same tool from Movement 1
...
- That output is the **path made visible**: it confirms the route was developer-determined. It's
  the same built-in call you used on the single node in Movement 1 — nothing new to set up.
- Routing this same run to **Langfuse** for a structured, comparable trace is
  **[L12](../L12/objectives.md)'s** job, several lessons away. Nothing here depends on tracing
  being set up.
