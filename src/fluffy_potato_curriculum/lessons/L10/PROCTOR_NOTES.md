...
>
> **Why the raw Anthropic SDK in L1006 (not `potato_llm`)?** The course's `potato_llm` seam is
> text-only — its `Message` cannot carry `tool_use`/`tool_result` blocks, which are exactly what the
> agent loop is built on. So L1006 calls the raw SDK directly; the key still loads through
> `common.config` (`require_anthropic_key`), never hard-coded. This is the same seam exception L07
