...
Covers both L12 labs: **L1204** (build the shallow agent as a graph) and **L1206** (graph state and
...
reducer**, not model output. The lecture demos ([L1203](L1203_lecture.ipynb),
[L1205](L1205_lecture.ipynb), [L1207](L1207_lecture.ipynb)) use the real `ChatAnthropic` client where
they make live calls; the labs deliberately don't, so the wiring and state are the only variables.

> Keep repeating the lesson's spine: **it's the same model → tool → model loop from L07, now drawn as
> a graph.** The one new thing versus an L11 workflow is the **back-edge** `tools → agent`, which
> hands the model control of the path. If a student can't map a graph piece back to an L07 line,
...
- Forgetting the `add_messages` reducer — writing `messages: list[BaseMessage]` instead of
  ...
  **overwrites**, the history gets clobbered, and the agent loops to the recursion cap. They'll see
  this break on purpose in L1206 — name the link now.
...
UNBLOCKERS: Point at the L1203/L1205 state schema — two fields, only `messages` is annotated. A
...
UNBLOCKERS: Have them say each line's L07 equivalent aloud: `agent_node` = "call the model",
...
error `ToolMessage` — L07's `is_error`).
...
arrow, the wiring is right. Compare it to the L1203 diagram.
...
UNBLOCKERS: If the run hits a recursion error, the reducer or the route is wrong — jump to the L1206
...
- Answering "the conditional edge" for Q1. Close, but the precise answer is the **back-edge**
  `tools → agent`: a conditional edge alone (as in L11) is still a workflow; the *cycle* is what
...
UNBLOCKERS: Point back to L11's "the line between a workflow and an agent is a single back-edge."
...
COMMON GOTCHAS:
- Same as L1204 problem 1: forgetting `add_messages`. Here it matters even more — problem 3 *depends*
...
UNBLOCKERS: This is the single most important beat of the lab — the L07 invariant bug, reborn in
...
todo list — the L16/L17 forward pointer).
