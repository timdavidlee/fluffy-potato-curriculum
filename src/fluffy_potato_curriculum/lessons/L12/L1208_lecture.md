# From one loop to many patterns: the bridge to L13

```yaml
title: "From one loop to many patterns: the bridge to L13"
...
> **Lesson:** L12. **Roadmap:** the optional forward-pointer in [demos_or_activities.md](../../../../docs/origin/lesson_roadmaps/L12/demos_or_activities.md).
> A short closing lecture — **diagram + discussion, no live build**. It reuses the compiled agent
> from the demos ([L1203](L1203_lecture.ipynb)) and *names* what comes next. Building the patterns is
> **L13's** job; don't teach them here.

## section 1. Recap — what you built

### slide 1.1 A shallow agent, as a graph

- You rebuilt the L07 hand-rolled loop as a **LangGraph graph**: an `agent` node, a `tools` node, a
...
- You also **evaluated** it (the L09 set passed unchanged) and **traced** it (the run landed in the
  L08 Langfuse). The skills travelled with the agent, not the implementation.

### slide 1.2 The primitives you now own

- table: the L12 vocabulary — every one of these is what L13 builds *patterns* on top of.
...
- In [L1203](L1203_lecture.ipynb) you hand-built the graph, then revealed
...
- That whole-graph one-liner is the **ReAct** pattern: **reason** (the model decides) →
  **act** (call a tool) → observe → repeat. It is a **pattern over the primitives you just built**,
  not a new mechanism.
- The point of building it by hand first: when L13 says "ReAct," you already know it is your
...
- ReAct is the first and simplest. The moment your control flow stops being one loop, other shapes
  earn their keep — which is exactly where L13 begins.

## section 3. The open question L13 answers

### slide 3.1 What else can this graph shape express?

- L12 owns **one graph, one loop, state mechanics.** L13 owns the **named patterns** and their
...
- diagram: the single shallow-agent loop on the left; on the right, four small unlabeled graph
  ...
  shrug-emoji "what else?" — captioned "L13."

### slide 3.2 …and when *not* to reach for a pattern

- Carry the L12 caution forward: a fancier pattern is more cost, more latency, more to debug.
  **Reach for the simplest shape that solves the task** — often that's the shallow loop you already
  have, or even the plain L07 function before it.
...
### slide 4.1 You built the primitive; L13 builds the patterns

- **A shallow agent is a single tool-calling loop expressed as a graph; the patterns in L13 are
...
- Everything carries forward unchanged — `StateGraph`, nodes, edges, typed state, reducers,
  `compile`/`invoke`, the eval set, the Langfuse hookup. L13 adds *shapes*, not new machinery.
