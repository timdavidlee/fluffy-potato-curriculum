...
> **Read in order:** this intro (the map) → `L1202_lecture` (read a trace, locate a failure) → `L1203_lab` → `L1204_lecture` (instrument the loop, compare two runs) → `L1205_lab` → `L1206_lecture` (see it in Langfuse; where extracts go instead).
...
The L10 loop and tools now live as a shared, reusable reference in `fluffy_potato_curriculum.common` — `common/agent_loop.py` (`run()` → `RunResult`), `common/tools.py` (`calculator`, `lookup`, `flaky_fetch`), and the new `common/tracing.py` (`TraceEvent`). L12 is the first lesson that *imports* them instead of hand-building them, because this lesson is about *observing* the loop, not re-deriving it. The labs drive that loop with a scripted `FakeModel` so they run deterministically with no API key.
