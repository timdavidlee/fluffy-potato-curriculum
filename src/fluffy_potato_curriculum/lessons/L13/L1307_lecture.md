...
- diagram: two-layer stack with tie-lines (the L12 hand-rolled→platform payoff picture, re-drawn for evals): bottom layer **dashed ink-faint** — `common/evals.py` (`EvalCase` · `Scorer` · `evaluate()` / `compare()`), labeled "readable concept sketch, not the runner"; top layer **cyan** — the self-hosted Langfuse (dataset · experiments · run comparison · managed judge). **Cyan** tie-lines pair each hand-rolled name with the platform object it *is*: `EvalCase` ↔ dataset item, `EvalResult` ↔ score, a run ↔ an experiment. Explicitly no coral — nothing on this slide fails.
